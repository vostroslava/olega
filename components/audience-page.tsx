...
const content = {
  ...
  business: {
    ...
    intro: "Инженерный подход для объекта любого масштаба",
    ...
    facts: [
      ["Техподдержка", "Работаем с архитектором, генподрядчиком и службой эксплуатации в одной рабочей связке."],
...
export function AudiencePage({ kind }: AudiencePageProps) {
  ...
  return (
    <div className="page-shell">
        ...
        <section className="monolith-content-section" id="section-3">
          <div className="container">
            ...
            <div className="monolith-process-line">
              {["Исходные данные и задача", "Замер и проверка узлов", "Проектирование и расчёт", "Производство и монтаж"].map((item, index) => (
                <article className="reveal" key={item}><span>{String(index + 1).padStart(2, "0")}</span><h3>{item}</h3><p>{index === 0 ? "Понимаем задачу, сроки и архитектурный замысел." : index === 1 ? "Фиксируем фактические размеры и риски на объекте." : index === 2 ? "Подбираем систему, стеклопакет, фурнитуру и монтажный узел." : "Изготавливаем, доставляем, монтируем и передаём гарантию."}</p></article>
              ))}
            </div>
          </div>
        </section>
      ...
      <SiteFooter />
      <RevealInit />
      <MobileCta heroId="audience-hero" requestId="request" href="/raschet/" label={kind === "home" ? "Рассчитать остекление" : "Обсудить объект"} />
    </div>
  );
}
