  ...
  return (
    <div className="page-shell">
      ...
      <main className="page-main monolith-inner-page">
        <MonolithPageHero id="privacy-hero" breadcrumbs={[{ label: "Главная", href: "/" }, { label: "Конфиденциальность" }]} title="Политика конфиденциальности" lead="Какие данные мы получаем через формы сайта, зачем используем и как можно управлять их обработкой." image="/assets/photos/project-arbat.png" imageAlt="Реальный объект СтеклоСтройГрупп" rail={["Данные", "Цели", "Хранение", "Аналитика", "Права", "Контакты"]} primaryLabel="Контакты" primaryHref="/kontakty/" secondaryLabel="На главную" secondaryHref="/" />
        <section className="legal-section" id="section-1">
          <div className="container legal-layout">
            ...
            <div className="legal-content reveal">
              ...
              <section><span>04</span><h2>Аналитика посещений</h2><p>Если пользователь разрешает статистику, сайт может передавать в подключённые системы аналитики сведения о посещении страниц, нажатиях на контакты и отправке форм. Текст обращений, телефоны, имена и прикреплённые файлы в события аналитики не передаются.</p></section>
              <section><span>05</span><h2>Права пользователя</h2><p>Вы можете запросить уточнение, изменение или удаление данных, а также отозвать согласие на обработку по email {CONTACTS.primaryEmail}.</p></section>
              <section><span>06</span><h2>Контакты</h2><p>Email: {CONTACTS.primaryEmail}<br />Телефон: {CONTACTS.phones[0].label}<br />Адрес: {CONTACTS.postalAddress}</p></section>
            </div>
          </div>
        </section>
      </main>
